...
class Amazon_S3_And_CloudFront extends AWS_Plugin_Base {
	...
	function __construct( $plugin_file_path, $aws ) {
		...
		$this->plugin_title = __( 'CLC Object Storage', 'as3cf' );
		$this->plugin_menu_title = __( 'CLC Object Storage', 'as3cf' );
	...
	function get_attachment_url( $post_id, $expires = null ) {
		...
		elseif ( is_ssl() || $this->get_setting( 'force-ssl' ) ) {
			$domain_bucket = 'ca.tier3.io/' . $s3object['bucket'];
			//$domain_bucket = AWS_REGION . '/' . $s3object['bucket'];
		}
		else {
			//$domain_bucket = $s3object['bucket'] . '.ca.tier3.io';
			$domain_bucket = $s3object['bucket'] . AWS_REGION;
		}
